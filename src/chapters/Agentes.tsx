...
import { AgentWorkflowSimulator } from '../components/InteractiveComponents';
...
export const AgentesContent = () => (
  <div className="prose-custom">
    ...
    <p className="text-xl text-slate-600 dark:text-slate-300 mb-8">Qué son, cómo funcionan y por qué importan en la automatización moderna.</p>

    <h2 id="sec-agentes-0" className="text-2xl font-bold mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="my-8 grid md:grid-cols-2 gap-6">
      <div className="bg-slate-100 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50">
        <h4 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-3">Chatbot básico</h4>
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-2xl border border-indigo-200 dark:border-indigo-500/30">
        <h4 className="text-lg font-bold text-indigo-900 dark:text-indigo-300 mb-3">Agente de IA</h4>
        <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-300 list-disc pl-4">
    ...
    <h2 id="sec-agentes-1" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
      ...
      <table className="w-full text-left border-collapse border border-slate-200 dark:border-slate-700">
        <thead>
          <tr className="bg-slate-100 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
            <th className="p-4 font-bold text-indigo-800 dark:text-indigo-300">Modelos</th>
            <th className="p-4 font-bold text-emerald-800 dark:text-emerald-300">Agentes</th>
          </tr>
        </thead>
    ...
    <h2 id="sec-agentes-2" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="space-y-4 my-6">
      <div className="flex items-start gap-4 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-white/5">
        <div className="p-2 bg-blue-500/20 rounded-lg text-blue-600 dark:text-blue-400 mt-1"><Zap size={20} /></div>
        <div>
          <h4 className="font-bold text-slate-800 dark:text-slate-200">1. Sensores (percepción)</h4>
          <p className="text-sm text-slate-600 dark:text-slate-400">Formas de percibir su entorno. Para los agentes de software, los "sensores" suelen ser los datos de entrada (texto del usuario, respuestas de una API, lectura de documentos).</p>
        </div>
      </div>
      <div className="flex items-start gap-4 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-white/5">
        <div className="p-2 bg-purple-500/20 rounded-lg text-purple-600 dark:text-purple-400 mt-1"><BrainCircuit size={20} /></div>
        <div>
          <h4 className="font-bold text-slate-800 dark:text-slate-200">2. Motor de razonamiento (cerebro)</h4>
          <p className="text-sm text-slate-600 dark:text-slate-400">La inteligencia interna que procesa las entradas y decide las acciones. En los agentes modernos, suele ser un <GlossaryTerm term="LLM">LLM</GlossaryTerm> (Gran Modelo de Lenguaje) que actúa como el motor central de lógica y planificación.</p>
        </div>
      </div>
      <div className="flex items-start gap-4 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-white/5">
        <div className="p-2 bg-emerald-500/20 rounded-lg text-emerald-600 dark:text-emerald-400 mt-1"><Layers size={20} /></div>
        <div>
          <h4 className="font-bold text-slate-800 dark:text-slate-200">3. Actuadores (herramientas)</h4>
          <p className="text-sm text-slate-600 dark:text-slate-400">Formas de actuar sobre el entorno. En lugar de mover partes físicas, un agente de software utiliza herramientas: enviar un correo, hacer una consulta SQL, buscar en la web o actualizar un CRM.</p>
        </div>
      </div>
    </div>

    <h2 id="sec-agentes-3" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="space-y-6 my-8">
      <div className="bg-slate-100 dark:bg-slate-800/30 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-indigo-500/20 rounded-lg text-indigo-700 dark:text-indigo-400"><Network size={24} /></div>
          <h4 className="text-xl font-bold text-indigo-900 dark:text-indigo-300">1. Extensiones (APIs externas)</h4>
        </div>
      ...
      <div className="bg-slate-100 dark:bg-slate-800/30 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-emerald-500/20 rounded-lg text-emerald-700 dark:text-emerald-400"><TerminalSquare size={24} /></div>
          <h4 className="text-xl font-bold text-emerald-900 dark:text-emerald-300">2. Funciones (function calling)</h4>
        </div>
        <p className="text-slate-700 dark:text-slate-300 mb-4">
      ...
      <div className="bg-slate-100 dark:bg-slate-800/30 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-amber-500/20 rounded-lg text-amber-700 dark:text-amber-400"><Database size={24} /></div>
          <h4 className="text-xl font-bold text-amber-900 dark:text-amber-300">3. Almacenes de datos (RAG)</h4>
        </div>
        <p className="text-slate-700 dark:text-slate-300 mb-4">
        ...
        <div className="bg-white dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-white/5">
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Ejemplo práctico: Soporte técnico interno</p>
          <p className="text-sm text-slate-600 dark:text-slate-400 italic">
    ...
    <h2 id="sec-agentes-5" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
      <Target size={24} /> ¿Cuándo necesitas realmente un agente?
    </h2>
    <p>
      Construir agentes requiere repensar cómo los sistemas toman decisiones. A diferencia de la automatización convencional (basada en reglas strictly), los agentes son ideales para flujos de trabajo donde los enfoques deterministas fallan. Considera construir un agente si tu caso de uso cumple con estos criterios:
    </p>
    ...
    <h2 id="sec-agentes-6" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="overflow-x-auto my-8">
      <table className="w-full text-left border-collapse bg-slate-100 dark:bg-slate-900/50 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
        <thead>
    ...
    <h2 id="sec-agentes-7" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <ul className="list-disc pl-5 space-y-2 mt-4 mb-6 text-slate-700 dark:text-slate-300">
...
agent.run(prompt)`}
    ...
    <h2 id="sec-agentes-9" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="grid sm:grid-cols-2 gap-4 my-6">
      ...
      <div className="p-5 bg-slate-100 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/50">
        <Briefcase className="text-emerald-600 dark:text-emerald-400 mb-3" size={24} />
        <h4 className="font-bold text-slate-900 dark:text-slate-200 mb-2">Atención al cliente autónoma</h4>
        <p className="text-sm text-slate-600 dark:text-slate-400">Agentes que no solo responden FAQs, sino que pueden procesar devoluciones, verificar el estado de un pedido en la base de datos y emitir reembolsos, escalando a un humano solo en casos complejos.</p>
      </div>
      <div className="p-5 bg-slate-100 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/50">
        <LineChart className="text-amber-600 dark:text-amber-400 mb-3" size={24} />
        <h4 className="font-bold text-slate-900 dark:text-slate-200 mb-2">Análisis de datos</h4>
        <p className="text-sm text-slate-600 dark:text-slate-400">Un agente al que le preguntas en lenguaje natural: "¿Cuáles fueron las ventas del Q3?". El agente traduce esto a una consulta SQL, extrae los datos de la base de datos, y genera un resumen ejecutivo con gráficos.</p>
      </div>
      <div className="p-5 bg-slate-100 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/50">
        <Layers className="text-pink-600 dark:text-pink-400 mb-3" size={24} />
        <h4 className="font-bold text-slate-900 dark:text-slate-200 mb-2">Curaduría e investigación</h4>
        <p className="text-sm text-slate-600 dark:text-slate-400">Agentes que monitorean constantemente la web o redes sociales en busca de temas específicos, filtran el spam, resumen los hallazgos relevantes y envían un reporte diario automatizado.</p>
      </div>
    </div>

    <h2 id="sec-agentes-10" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <InfoCard type="reflexion" title="Supervisión ética con DECIDE-IA y PotencIA">
      <p>El principio de Human-in-the-Loop (HITL) es el núcleo del <GlossaryTerm term="DECIDE-IA"><strong>Marco DECIDE-IA</strong></GlossaryTerm> (Responsabilidad profesional y agencia humana) y del <GlossaryTerm term="PotencIA"><strong>Marco PotencIA</strong></GlossaryTerm>. Garantiza que la o el <GlossaryTerm term="Docente de educación secundaria"><strong>docente de educación secundaria</strong></GlossaryTerm> mantenga siempre la autonomía y supervisión ética sobre las decisiones didácticas y los flujos agénticos automatizados.</p>
    </InfoCard>

    <h2 id="sec-agentes-12" className="text-2xl font-bold mt-12 mb-4 text-indigo-800 dark:text-indigo-300 scroll-mt-24 flex items-center gap-2">
    ...
    <div className="my-8 relative">
      ...
      <div className="space-y-8">
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start">
          <div className="hidden md:flex items-center justify-center w-16 h-16 rounded-full bg-white dark:bg-slate-900 border-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 z-10 shrink-0 shadow-md">
            <Layers size={24} />
          </div>
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="bg-slate-100 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50 flex-1">
            ...
            <div className="bg-white dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 text-sm">
              <strong className="text-emerald-700 dark:text-emerald-400">Agente "Buscador de Oportunidades":</strong> Recibe el perfil de SofIA y utiliza <em>Extensiones</em> (como la API de LinkedIn o SerpAPI para Google Jobs) para buscar proyectos EdTech abiertos en las últimas 24 horas.
            </div>
          </div>
        </div>
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="bg-slate-100 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50 flex-1">
            ...
            <div className="bg-white dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 text-sm">
              <strong className="text-emerald-700 dark:text-emerald-400">Agente "Orquestador":</strong> Utiliza un marco de razonamiento (como ReAct) para comparar las 50 oportunidades con el perfil de SofIA. Descarta las que requieren presencialidad (si SofIA pidió remoto) y selecciona el "Top 3" con mayor porcentaje de coincidencia.
            </div>
          </div>
        </div>
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="bg-slate-100 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50 flex-1">
            <h4 className="text-lg font-bold text-indigo-800 dark:text-indigo-300 mb-2">Paso 5: Human-in-the-Loop (HITL)</h4>
            <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">
              Antes de enviar cualquier propuesta real, el sistema se detiene.
            </p>
            <div className="bg-white dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 text-sm">
              <strong className="text-emerald-700 dark:text-emerald-400">SofIA (El Humano):</strong> Revisa las 3 propuestas y los borradores de las cartas en su panel de control. Modifica un párrafo de una carta y hace clic en "Aprobar y Enviar" solo para 2 de las oportunidades. El agente entonces ejecuta la acción final de envío.
            </div>
          </div>
        </div>
      </div>
