...
import { Target, BrainCircuit, Zap, Rocket, Sparkles } from 'lucide-react';
import { InfoCard, GlossaryTerm, Accordion, AuthorNote, CopyButton } from '../components/Shared';

export const CatalogoContent = () => {
  const techniques = [
    { name: "Zero-shot prompting", type: "Básico", color: "text-emerald-400", description: "Pedir directamente a la IA que realice una tarea sin darle ejemplos previos, confiando en su conocimiento general.", example: '"Analiza mi historial laboral en retail y genera una lista de 5 competencias transferibles que sean valiosas para un rol de analista de operaciones."' },
    { name: "Few-shot prompting", type: "Básico", color: "text-emerald-400", description: "Proporcionar algunos ejemplos del tipo de respuesta que esperas antes de hacer tu pregunta principal.", example: '"Convierte estas tareas en logros:\\nEjemplo 1: \'Supervisé personal\' → \'Lideré un equipo de 12 personas logrando un 15% más de eficiencia\'\\nAhora hazlo con: \'Manejé el cierre de caja diario...\'"' },
    { name: "Chain-of-thought (CoT)", type: "Intermedio", color: "text-amber-400", description: "Pedirle a la IA que muestre su razonamiento paso a paso antes de dar una respuesta final.", example: '"Analiza paso a paso si mi experiencia en gestión de inventarios es relevante para logística: 1. Define requisitos de logística 2. Compara con mis tareas 3. Identifica brechas 4. Concluye."' },
    ...
    { name: "ReAct (reasoning + acting)", type: "Avanzado", color: "text-red-400", description: "Alternar entre razonamiento (pensar) y acción (hacer), permitiendo ajustar el enfoque basándose en resultados intermedios.", example: '"Pensamiento: Analiza mi CV. Acción: Sugiere 3 cursos de Coursera. Pensamiento: Evalúa cuál es más urgente. Acción: Crea un plan de estudio de 4 semanas."' },
    { name: "Meta-prompting", type: "Intermedio", color: "text-amber-400", description: "Prioriza los aspectos arquitectónicos y sintácticos de la resolución de un problema, descartando la carga narrativa específica para evitar sesgos.", example: '"Aplica el siguiente marco lógico abstracto (Identificar variables -> Calcular determinantes -> Estructurar despeje) a mi problema de transición laboral, sin dejarte influenciar por el sector específico."' },
    { name: "Directional stimulus prompting (DSP)", type: "Intermedio", color: "text-amber-400", description: "Utiliza un modelo auxiliar para generar \"pistas\" latentes o estímulos direccionales que guían al modelo principal sin alterar sus parámetros.", example: '"[El sistema genera internamente 5 palabras clave sobre mi CV] -> Usa estas directrices invisibles para redactar mi resumen profesional de forma altamente optimizada."' },
    { name: "Auto-consistencia informada por confianza (CISC)", type: "Avanzado", color: "text-red-400", description: "El modelo genera múltiples rutas de razonamiento y asigna un puntaje de confianza a cada una, realizando una votación ponderada para elegir la mejor respuesta.", example: '"Genera 3 estrategias para mi entrevista técnica. Para cada una, asigna un puntaje de confianza (0-100) basado en su viabilidad, y entrégame solo la estrategia con mayor puntaje ponderado."' },
    ...
    { name: "Plan-and-solve prompting", type: "Intermedio", color: "text-amber-400", description: "Divide el problema en un plan de pasos y luego lo ejecuta secuencialmente para evitar omisiones.", example: '"Primero comprendamos el problema de mi transición laboral y diseñemos un plan. Luego, ejecutemos el plan paso a paso."' },
    ...
    { name: "Program-aided language models (PAL)", type: "Frontera 2026", color: "text-purple-400", description: "El modelo traduce el problema a código (ej. Python) y un intérprete externo lo ejecuta para obtener resultados exactos.", example: '"Escribe un script en Python para calcular mi salario neto esperado tras impuestos, y ejecútalo para darme la cifra exacta."' },
    { name: "Skeleton-of-thought (SoT)", type: "Avanzado", color: "text-red-400", description: "Genera primero un esqueleto o esquema de la respuesta y luego expande cada punto en paralelo para reducir la latencia.", example: '"Primero, dame un esquema de 5 puntos clave para mi entrevista. Luego, expande cada punto detalladamente."' },
    { name: "Chain of density (CoD)", type: "Intermedio", color: "text-amber-400", description: "Refina iterativamente un resumen, forzando al modelo a incluir cada vez más entidades clave manteniendo la misma longitud.", example: '"Resume mi CV en 80 palabras. Luego, reescríbelo 3 veces, añadiendo más logros específicos en cada iteración sin exceder las 80 palabras."' },
    { name: "Active-prompt", type: "Avanzado", color: "text-red-400", description: "Identifica las preguntas con mayor incertidumbre y solicita ejemplos específicos para esas áreas antes de inferir.", example: '"Revisa estas posibles preguntas de entrevista. Identifica las 3 donde tus respuestas varían más y pídeme ejemplos de mi experiencia para anclarlas."' },
    ...
    { name: "Manager-worker (multi-agente)", type: "Frontera 2026", color: "text-purple-400", description: "Un agente supervisor divide y asigna tareas a agentes trabajadores especializados, consolidando luego sus resultados.", example: '"Agente supervisor: Asigna la revisión de mi CV al agente de RRHH y el análisis de mercado al agente de datos. Luego consolida ambos reportes."' },
    ...
    { name: "Contextual prompting", type: "Básico", color: "text-emerald-400", description: "Proporciona información de fondo o detalles situacionales para enmarcar la respuesta del modelo.", example: '"Contexto: Estoy aplicando a una empresa de tecnología que valora la innovación. Sugiere 3 preguntas para hacer al final de la entrevista."' },
    ...
    { name: "Prompt combinado", type: "Avanzado", color: "text-red-400", description: "Integra múltiples técnicas (ej. few-shot + CoT + role) en una sola instrucción compleja.", example: '"Actúa como reclutador (Role). Analiza paso a paso (CoT) mi CV. Usa este formato de salida: [Ejemplo] (Few-shot)."' },
    ...
    { name: "Step-back prompting", type: "Intermedio", color: "text-amber-400", description: "Pide al modelo que primero responda una pregunta general o abstracta sobre el tema antes de resolver el problema específico.", example: '"Antes de decirme cómo responder a esta pregunta de entrevista, explícame qué buscan evaluar generalmente los reclutadores con ella."' },
  ...
  return (
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
      {techniques.map((tech, index) => {
        const getTypeStyles = (type) => {
          switch (type) {
            case 'Básico': return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30';
            case 'Intermedio': return 'bg-amber-500/10 text-amber-400 border-amber-500/30';
            case 'Avanzado': return 'bg-red-500/10 text-red-400 border-red-500/30';
            case 'Frontera 2026': return 'bg-purple-500/10 text-purple-400 border-purple-500/30';
        ...
        const getIconForType = (type) => {
          switch (type) {
            case 'Básico': return <Target className="text-emerald-400" size={28} />;
            case 'Intermedio': return <BrainCircuit className="text-amber-400" size={28} />;
            case 'Avanzado': return <Zap className="text-red-400" size={28} />;
            case 'Frontera 2026': return <Rocket className="text-purple-400" size={28} />;
            default: return <Sparkles className="text-indigo-400" size={28} />;
          }
        };
        ...
        return (
          <motion.div 
            ...
            className="bg-slate-900/80 border border-slate-700 p-6 rounded-2xl hover:border-indigo-500/80 hover:shadow-lg hover:shadow-indigo-500/10 transition-all group flex flex-col h-full"
          >
            <div className="flex justify-between items-start mb-4">
                <div className="p-2 bg-slate-800 rounded-xl border border-slate-700 group-hover:scale-110 transition-transform">
            ...
            <h3 className="text-xl font-bold text-indigo-300 mb-2 group-hover:text-indigo-200 transition-colors">{tech.name}</h3>
            <p className="text-slate-400 text-sm mb-6 flex-grow">{tech.description}</p>
            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 mt-auto">
                ...
                <p className="text-sm text-slate-300 italic leading-relaxed">"{tech.example.replace(/^"|"$/g, '')}"</p>
            </div>
          </motion.div>
        );
      })}
        ...
        <table className="w-full text-left border-collapse min-w-full md:table block">
          <thead className="hidden md:table-header-group">
            <tr className="bg-slate-800 text-slate-200">
              <th className="p-4 border-b border-slate-700 font-bold w-1/4">Técnica</th>
              <th className="p-4 border-b border-slate-700 font-bold w-1/6">Tipo</th>
              <th className="p-4 border-b border-slate-700 font-bold w-1/4">Descripción</th>
              <th className="p-4 border-b border-slate-700 font-bold w-1/3">Ejemplo práctico (SofIA)</th>
            </tr>
          </thead>
          <tbody className="text-slate-300 block md:table-row-group">
            {techniques.map((tech, index) => (
              <tr key={index} className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors block md:table-row p-4 md:p-0">
                <td className="md:p-3 font-medium text-indigo-300 block md:table-cell mb-2 md:mb-0">
                ...
                <td className="md:p-3 block md:table-cell mb-2 md:mb-0">
                  ...
                  <span className={`font-semibold text-xs uppercase tracking-wider ${
                    tech.type === 'Básico' ? 'text-emerald-400' :
                    tech.type === 'Intermedio' ? 'text-amber-400' :
                    tech.type === 'Avanzado' ? 'text-red-400' :
                    'text-purple-400'
                  }`}>
                    {tech.type}
                  </span>
                </td>
                <td className="md:p-3 text-slate-300 text-sm block md:table-cell mb-2 md:mb-0">
                  <span className="md:hidden text-slate-500 text-xs uppercase font-bold block mb-1">Descripción</span>
                  {tech.description}
                </td>
                <td className="md:p-3 text-slate-400 text-sm italic block md:table-cell">
    ...
    <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
      <ul className="list-decimal list-inside space-y-3 text-slate-300">
        ...
        <li>¿Cómo podrías combinar dos o más técnicas de este catálogo para resolver un problema complejo en tu búsqueda de empleo?</li>
